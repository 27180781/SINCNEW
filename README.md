...
כשהמתקשר חוזר לימות אחרי הצינתוק, שלוחת API קוראת ל-`GET /api/get-intro-text?ApiPhone=<מספר>`
והמערכת מחזירה בפורמט ההקראה של ימות — `read=t-<שורה1>.t-<שורה2>...` (כל שורה קטע נפרד).
להוספת `&format=text` מחזיר טקסט גולמי לתצוגה. הטקסט לפי התוצאה האחרונה של אותו מספר:
